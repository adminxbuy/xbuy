...
<div class="bg-zinc-50 min-h-[calc(100vh-120px)] py-10">
 ...
 <a href="{{ route('dashboard.orders', ['order_type' => 'sold']) }}"  class="px-2 py-2 rounded-sm text-sm {{ $type === 'sold' ? 'font-bold text-zinc-900 bg-zinc-100' : 'font-medium text-zinc-700 hover:bg-zinc-50' }}">
 Sold
 </a>
 <a href="{{ route('dashboard.orders', ['order_type' => 'bought']) }}"  class="px-2 py-2 rounded-sm text-sm {{ $type === 'bought' ? 'font-bold text-zinc-900 bg-zinc-100' : 'font-medium text-zinc-700 hover:bg-zinc-50' }}">
 Bought
 </a>
 </nav>
 </div>
 ...
 <span class="inline-block px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider"  style="background-color: {{ $order->status_color }}15; color: {{ $order->status_color }}">
 {{ $order->status_label }}
 </span>
 </div>
