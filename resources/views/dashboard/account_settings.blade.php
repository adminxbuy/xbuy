...
<div class="bg-zinc-50 min-h-[calc(100vh-120px)] py-10">
 ...
 <!-- Right Content Area -->
 <div class="flex-1 space-y-8" x-data="{  vacationMode: {{ $user->vacation_mode ? 'true' : 'false' }},  editingEmail: false,  emailVal: '{{ old('email', $user->email) }}',
 editingPhone: false,
 ...
 body: JSON.stringify({  phone: '+91' + this.phoneVal.trim().replace(/[^0-9]/g, '').slice(-10),
 ...
 setTimeout(() => {  this.showOtpModal = false;  this.otpSuccess = '';  this.otpCode = '';  window.location.reload();
 }, 1500);
 } else {
 ...
 <button type="button"  @click="vacationMode = !vacationMode"  class="w-12 h-6 rounded-full p-0.5 focus:outline-none cursor-pointer"
