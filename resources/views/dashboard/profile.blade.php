...
<div class="relative bg-zinc-50 z-10 min-h-[calc(100vh-120px)] py-8">
 ...
 <button type="button"  @click="navigator.clipboard.writeText('{{ route('member.profile', $user->profile_id) }}'); copied = true; setTimeout(() => copied = false, 2000)"
 ...
 <button @click="activeTab = 'listings'"  class="pb-3 border-b-2 text-sm focus:outline-none cursor-pointer"
 :class="activeTab === 'listings' ? 'border-[#e6c019] text-zinc-900 font-bold' : 'border-transparent text-zinc-400 font-medium'">
 Listings
 </button>
 <button @click="activeTab = 'reviews'"  class="pb-3 border-b-2 text-sm focus:outline-none cursor-pointer"
 ...
 <a href="/dashboard/listings/create"  class="mt-6 inline-flex items-center justify-center px-8 py-3 rounded-xl text-sm font-bold text-black bg-[#fdd835] border border-black/10">
 List now
 </a>
 @else
