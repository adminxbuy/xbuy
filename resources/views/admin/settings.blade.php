...
@section('content')
<div class="bg-card border border-border rounded-xl ring-0 overflow-hidden p-6" x-data="{ 
    tab: 'general', 
    testEmail: '', 
    testSuccess: '', 
    testError: '', 
    ...
    <div class="flex border-b border-border mb-6 overflow-x-auto pb-1 gap-1">
        @foreach([
            ...
            <button type="button" @click="tab = '{{ $tInfo['id'] }}'" 
                    :class="tab === '{{ $tInfo['id'] }}' ? 'border-foreground font-semibold text-foreground' : 'border-transparent text-muted-foreground hover:text-foreground'" 
