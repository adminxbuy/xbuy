...
<div class="space-y-6" x-data="usersPage()">
    ...
    <div class="flex flex-wrap gap-2.5">
        <a href="{{ route('admin.users') }}"
           class="text-xs font-semibold px-4 py-2.5 rounded-xl border transition-all {{ !request('badge') && request('tab') !== 'deleted' ? 'bg-primary text-primary-foreground border-border shadow-sm' : 'bg-card border-border text-muted-foreground hover:bg-muted' }}">
            All Buyers
        </a>
        <a href="{{ route('admin.users', ['badge' => 'new_buyer']) }}"
           class="text-xs font-semibold px-4 py-2.5 rounded-xl border transition-all {{ request('badge') === 'new_buyer' && request('tab') !== 'deleted' ? 'bg-primary text-primary-foreground border-border shadow-sm' : 'bg-card border-border text-muted-foreground hover:bg-muted' }}">
            🆕 New Buyer ({{ $badgeCounts['new_buyer'] }})
        </a>
        <a href="{{ route('admin.users', ['badge' => 'verified_buyer']) }}"
           class="text-xs font-semibold px-4 py-2.5 rounded-xl border transition-all {{ request('badge') === 'verified_buyer' && request('tab') !== 'deleted' ? 'bg-primary text-primary-foreground border-border shadow-sm' : 'bg-card border-border text-muted-foreground hover:bg-muted' }}">
            🔵 Verified Buyer ({{ $badgeCounts['verified_buyer'] }})
        </a>
        <a href="{{ route('admin.users', ['badge' => 'trusted_buyer']) }}"
           class="text-xs font-semibold px-4 py-2.5 rounded-xl border transition-all {{ request('badge') === 'trusted_buyer' && request('tab') !== 'deleted' ? 'bg-primary text-primary-foreground border-border shadow-sm' : 'bg-card border-border text-muted-foreground hover:bg-muted' }}">
            🟢 Trusted Buyer ({{ $badgeCounts['trusted_buyer'] }})
        </a>
        <a href="{{ route('admin.users', ['tab' => 'deleted']) }}"
           class="text-xs font-semibold px-4 py-2.5 rounded-xl border transition-all {{ request('tab') === 'deleted' ? 'bg-primary text-primary-foreground border-border shadow-sm' : 'bg-card border-border text-muted-foreground hover:bg-muted' }}">
                            ...
                            <td class="p-4 pl-6">
                                <div class="flex items-center gap-2.5">
                                    <img src="https://ui-avatars.com/api/?name={{ urlencode($user->name) }}&background=e4e4e7&color=71717a&size=32"
                                         class="w-8 h-8 rounded-lg shrink-0 border border-border shadow-sm" alt="">
        ...
        <div class="relative w-full max-w-md mx-auto bg-card rounded-[24px] shadow-2xl border border-border z-10 overflow-hidden" 
            ...
            <form :action="'/admin/users/' + targetUser.id + '/' + actionType" method="POST" class="p-6 space-y-4 text-sm">
                ...
                <p class="text-xs text-muted-foreground leading-relaxed">
                    Are you sure you want to <span class="font-bold text-foreground" x-text="actionType"></span> the account of 
                    <span class="font-bold text-foreground" x-text="targetUser.name"></span> (<span x-text="targetUser.email"></span>)?
                </p>
                ...
                <div class="pt-4 border-t border-border flex justify-end space-x-2">
                    ...
                    <button type="submit" 
                            ...
                            class="px-5 py-2 text-primary-foreground font-semibold rounded-lg text-xs shadow-sm transition-all">
                        Confirm Action
                    </button>
                </div>
            </form>
            ...
            <div class="p-6 space-y-5 text-sm" x-show="kycUser">
                ...
                <div class="flex items-center gap-3 pb-4 border-b border-border" x-if="kycUser">
                    <img :src="'https://ui-avatars.com/api/?name=' + encodeURIComponent(kycUser.name) + '&background=e4e4e7&color=71717a&size=40'"
                         class="w-10 h-10 rounded-lg shrink-0 border border-border shadow-sm" alt="">
                    <div>
                ...
                <div class="space-y-4" x-show="kycUser && kycUser.wallet">
                    ...
                    <div class="pt-4 border-t border-border flex items-center justify-between">
                        ...
                        <div>
                            ...
                            <span :class="kycUser && kycUser.wallet && kycUser.wallet.is_activated ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-muted text-muted-foreground border border-border'"
                ...
                <div class="pt-5 border-t border-border flex justify-end">
                    <button type="button" @click="closeKycModal()" class="px-5 py-2.5 bg-primary text-primary-foreground font-semibold rounded-lg text-xs hover:bg-primary/90 transition-all shadow-sm">
                        Close
                    </button>
                </div>
            </div>
