...
<div class="space-y-6" x-data="usersPage()">
 ...
 class="text-xs font-semibold px-4 py-2.5 rounded-xl border transition-all {{ !request('badge') && request('tab') !== 'deleted' ? 'bg-primary text-primary-foreground border-border ' : 'bg-card border-border text-muted-foreground hover:bg-muted' }}">
 All Buyers
 </a>
 <a href="{{ route('admin.users', ['badge' => 'new_buyer']) }}"
 class="text-xs font-semibold px-4 py-2.5 rounded-xl border transition-all {{ request('badge') === 'new_buyer' && request('tab') !== 'deleted' ? 'bg-primary text-primary-foreground border-border ' : 'bg-card border-border text-muted-foreground hover:bg-muted' }}">
 🆕 New Buyer ({{ $badgeCounts['new_buyer'] }})
 </a>
 <a href="{{ route('admin.users', ['badge' => 'verified_buyer']) }}"
 class="text-xs font-semibold px-4 py-2.5 rounded-xl border transition-all {{ request('badge') === 'verified_buyer' && request('tab') !== 'deleted' ? 'bg-primary text-primary-foreground border-border ' : 'bg-card border-border text-muted-foreground hover:bg-muted' }}">
 🔵 Verified Buyer ({{ $badgeCounts['verified_buyer'] }})
 </a>
 <a href="{{ route('admin.users', ['badge' => 'trusted_buyer']) }}"
 class="text-xs font-semibold px-4 py-2.5 rounded-xl border transition-all {{ request('badge') === 'trusted_buyer' && request('tab') !== 'deleted' ? 'bg-primary text-primary-foreground border-border ' : 'bg-card border-border text-muted-foreground hover:bg-muted' }}">
 🟢 Trusted Buyer ({{ $badgeCounts['trusted_buyer'] }})
 </a>
 <a href="{{ route('admin.users', ['tab' => 'deleted']) }}"
 class="text-xs font-semibold px-4 py-2.5 rounded-xl border transition-all {{ request('tab') === 'deleted' ? 'bg-primary text-primary-foreground border-border ' : 'bg-card border-border text-muted-foreground hover:bg-muted' }}">
 ...
 <a href="{{ route('admin.users', request()->only('badge')) }}" class="px-4 py-2 border border-border text-muted-foreground hover:bg-muted rounded-xl text-xs font-semibold flex items-center transition-all">
 Clear Filters
 </a>
 @endif
 </div>
 ...
 class="w-8 h-8 rounded-lg shrink-0 border border-border " alt="">
 <span class="font-bold text-foreground">{{ $user->name }}</span>
 </div>
 </td>
 <!-- Email -->
 <td class="p-4 text-xs font-medium text-muted-foreground">{{ $user->email }}</td>
 <!-- Phone -->
 <td class="p-4 text-xs font-mono text-muted-foreground">{{ $user->phone ?? '—' }}</td>
 <!-- Location (Show City) -->
 ...
 <div class="mt-1">
 @if($user->show_city)
 <span class="text-[9px] font-bold px-1.5 py-0.5 bg-card text-foreground border border-border rounded-full">Shown</span>
 @else
 <span class="text-[9px] font-bold px-1.5 py-0.5 bg-muted text-muted-foreground border border-border rounded-full">Hidden</span>
 @endif
 </div>
 @else
 ...
 'trusted_buyer' => ['Trusted Buyer', 'bg-card text-foreground border border-border'],
 'verified_buyer' => ['Verified Buyer', 'bg-muted text-foreground border border-border'],
 ...
 <!-- Joined Date -->
 <td class="p-4 text-xs text-muted-foreground">{{ $user->created_at->format('d M Y') }}</td>
 <!-- Status -->
 <td class="p-4 text-center">
 @if($user->trashed())
 <span class="px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider rounded-full bg-muted text-foreground border border-border">
 Deleted
 </span>
 @elseif($user->status === 'active')
 @if($user->is_logged_in)
 <span class="px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider rounded-full bg-card text-foreground border border-border">
 Active
 </span>
 @else
 <span class="px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider rounded-full bg-muted text-muted-foreground border border-border">
 Inactive
 </span>
 @endif
 ...
 @else
 <span class="px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider rounded-full bg-muted text-foreground">
 {{ $user->status }}
 </span>
 @endif
 </td>
 ...
 @if($user->trashed())
 <span class="text-xs text-muted-foreground font-semibold italic" title="Data preserved under IT Act regulations">Data Preserved (IT Act)</span>
 @elseif($user->status === 'active')
 <button @click="openActionModal('suspend', {{ json_encode($user) }})" class="text-[10px] bg-muted text-foreground hover:bg-border border border-border px-2 py-1.5 rounded-lg transition-all font-bold">
 Suspend
 </button>
 <button @click="openActionModal('ban', {{ json_encode($user) }})" class="text-[10px] bg-muted text-foreground hover:bg-border border border-border px-2 py-1.5 rounded-lg transition-all font-medium">
 Ban
 </button>
 @else
 ...
 @empty
 <tr>
 <td colspan="9" class="p-12 text-center text-muted-foreground italic">
 ...
 <!-- Backdrop -->
 <div class="fixed inset-0 bg-background/65 backdrop-blur-md transition-opacity" @click="closeActionModal()"></div>
 ...
 Are you sure you want to <span class="font-bold text-foreground" x-text="actionType"></span> the account of  <span class="font-bold text-foreground" x-text="targetUser.name"></span> (<span x-text="targetUser.email"></span>)?
 </p>
 ...
 <button type="button" @click="closeActionModal()" class="px-4 py-2 border border-border text-muted-foreground hover:bg-muted font-semibold rounded-lg text-xs transition-all">
 Cancel
 </button>
 <button type="submit"  class="px-5 py-2 bg-primary text-primary-foreground hover:bg-primary/90 font-semibold rounded-lg text-xs transition-all">
 Confirm Action
 </button>
 </div>
 </form>
 ...
 <!-- Backdrop -->
 <div class="fixed inset-0 bg-background/65 backdrop-blur-md transition-opacity" @click="closeKycModal()"></div>
 ...
 <!-- User Basic Info -->
 <div class="flex items-center gap-3 pb-4 border-b border-border" x-if="kycUser">
 <img :src="'https://ui-avatars.com/api/?name=' + encodeURIComponent(kycUser.name) + '&background=e4e4e7&color=71717a&size=40'"
 class="w-10 h-10 rounded-lg shrink-0 border border-border " alt="">
 <div>
 <h5 class="font-bold text-foreground" x-text="kycUser.name"></h5>
 <p class="text-xs text-muted-foreground" x-text="kycUser.email"></p>
 </div>
 </div>
 ...
 <div>
 <span class="text-xs font-bold text-muted-foreground uppercase tracking-wider block">First Name</span>
 <span class="font-semibold text-foreground" x-text="kycUser && kycUser.wallet ? kycUser.wallet.first_name || '—' : '—'"></span>
 </div>
 <div>
 <span class="text-xs font-bold text-muted-foreground uppercase tracking-wider block">Last Name</span>
 <span class="font-semibold text-foreground" x-text="kycUser && kycUser.wallet ? kycUser.wallet.last_name || '—' : '—'"></span>
 </div>
 </div>

 <div class="grid grid-cols-2 gap-4">
 <div>
 <span class="text-xs font-bold text-muted-foreground uppercase tracking-wider block">Date of Birth</span>
 <span class="font-semibold text-foreground" x-text="kycUser && kycUser.wallet && kycUser.wallet.dob_day ? (kycUser.wallet.dob_day + ' ' + kycUser.wallet.dob_month + ' ' + kycUser.wallet.dob_year) : '—'"></span>
 </div>
 <div>
 <span class="text-xs font-bold text-muted-foreground uppercase tracking-wider block">SSN Last 4</span>
 <span class="font-semibold text-foreground font-mono" x-text="kycUser && kycUser.wallet ? kycUser.wallet.ssn_last_four || '—' : '—'"></span>
 </div>
 </div>

 <div>
 <span class="text-xs font-bold text-muted-foreground uppercase tracking-wider block">Billing Address</span>
 <span class="font-semibold text-foreground" x-text="kycUser ? kycUser.address || (kycUser.wallet ? kycUser.wallet.billing_address : '—') : '—'"></span>
 </div>

 <div class="pt-4 border-t border-border flex items-center justify-between">
 <div>
 <span class="text-xs font-bold text-muted-foreground uppercase tracking-wider block">Wallet Balance</span>
 <span class="font-extrabold text-foreground text-lg" x-text="kycUser && kycUser.wallet ? '₹' + parseFloat(kycUser.wallet.balance).toFixed(2) : '₹0.00'"></span>
 </div>
 <div>
 <span class="text-xs font-bold text-muted-foreground uppercase tracking-wider block mb-1">Activation Status</span>
 <span :class="kycUser && kycUser.wallet && kycUser.wallet.is_activated ? 'bg-card text-foreground border border-border' : 'bg-muted text-muted-foreground border border-border'"
 ...
 x-text="kycUser && kycUser.wallet && kycUser.wallet.is_activated ? 'Active' : 'Pending'">
  ...
  <div class="py-8 text-center text-muted-foreground italic" x-show="kycUser && !kycUser.wallet">
 No wallet details initialized for this user.
 </div>

 <div class="pt-5 border-t border-border flex justify-end">
 <button type="button" @click="closeKycModal()" class="px-5 py-2.5 bg-primary text-primary-foreground font-semibold rounded-lg text-xs hover:bg-primary/90 transition-all ">
 Close
 </button>
 </div>
 </div>
