    ...
    <div class="space-y-6" x-data="brandManager()">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div class="flex-1 w-full sm:w-auto">
                <p style="color: var(--muted-foreground);" class="text-sm">Add, edit, delete and organize brands and map them to parent categories.
                </p>
            </div>
        ...
        <!-- Filter & Search Bar -->
        <div
            class="rounded-xl p-4 flex flex-col md:flex-row gap-4 justify-between items-center" style="background: var(--muted); border: 1px solid var(--border);">
            <div class="relative w-full md:w-72">
                <input type="text" x-model="searchQuery" placeholder="Search brands..."
                    class="w-full pl-9 pr-4 py-2 rounded-xl focus:ring-1 focus:ring-ring focus:border-ring focus:ring-4 focus:ring-ring/15 focus:outline-none text-sm transition-all" style="background: var(--card); border: 1px solid var(--border); color: var(--foreground);">
            ...
            <div class="flex items-center space-x-2 w-full md:w-auto">
                <span class="text-xs font-semibold uppercase tracking-wider" style="color: var(--muted-foreground);">Filter Category:</span>
                <select x-model="categoryFilter"
                    class="p-2 rounded-xl text-sm focus:outline-none focus:ring-1 focus:ring-ring focus:border-ring" style="background: var(--card); border: 1px solid var(--border); color: var(--foreground);">
        ...
        <div style="background: var(--card); border: 1px solid var(--border);" class="rounded-xl ring-0 overflow-hidden">
            <div style="background: var(--muted); border-bottom: 1px solid var(--border);" class="p-5 flex justify-between items-center">
                <h3 class="font-bold text-sm flex items-center" style="color: var(--foreground);">
                    <i data-lucide="tag" class="w-4 h-4 mr-2" style="color: var(--muted-foreground);"></i>
                    All Brands List
                </h3>
                <span class="text-xs font-medium" style="color: var(--muted-foreground);" x-text="filteredBrandsCount() + ' brands found'"></span>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr
                            class="border-b text-[11px] font-bold uppercase tracking-wider" style="background: var(--muted); border-color: var(--border); color: var(--muted-foreground);">
                    ...
                    <tbody class="divide-y" style="border-color: var(--border);">
                        <template x-for="brand in filteredBrands()" :key="brand.id">
                            <tr class="hover:opacity-80 transition-colors text-sm" style="color: var(--foreground);">
                                <!-- Logo -->
                                <td class="p-4 pl-6">
                                    <div
                                        class="w-10 h-10 rounded-xl flex items-center justify-center overflow-hidden" style="background: var(--muted); border: 1px solid var(--border);">
                                ...
                                <!-- Brand Name -->
                                <td class="p-4">
                                    <span class="font-bold" style="color: var(--foreground);" x-text="brand.name"></span>
                                    <p class="text-[10px]" style="color: var(--muted-foreground);" x-text="brand.slug"></p>
                                </td>
                                <!-- Associated Categories -->
                                <td class="p-4">
                                    <div class="flex flex-wrap gap-1">
                                        <template x-if="brand.categories.length === 0">
                                            <span class="text-xs italic" style="color: var(--muted-foreground);">None</span>
                                        </template>
                                ...
                                <!-- Listing Count -->
                                <td class="p-4 font-semibold" style="color: var(--muted-foreground);" x-text="brand.listings_count ?? 0"></td>
                                <!-- Active Toggle -->
                                ...
                                <td class="p-4 pr-6 text-right space-x-1">
                                    <button @click="openEditModal(brand)"
                                        class="p-2 hover:opacity-70 rounded-xl transition-all"
                                        style="color: var(--muted-foreground);" title="Edit Brand">
                        ...
                        <tr x-show="filteredBrands().length === 0">
                            <td colspan="6" class="p-12 text-center italic" style="color: var(--muted-foreground);">
            ...
            <div class="relative w-full max-w-lg mx-auto rounded-[28px] shadow-2xl z-10 overflow-hidden" style="background: var(--card); border: 1px solid var(--border);"
                ...
                <div class="px-6 py-5 flex items-center justify-between" style="background: var(--muted); border-bottom: 1px solid var(--border);">
                    <div class="flex items-center space-x-2.5">
                        ...
                        <h4 class="font-bold text-base" style="color: var(--foreground);" x-text="isEdit ? 'Edit Brand' : 'Add New Brand'"></h4>
                    </div>
                ...
                <form :action="isEdit ? '/admin/brands/' + form.id : '{{ route('admin.brands.store') }}'" method="POST"
                    ...
                    <div class="space-y-1.5">
                        <label for="modal_name" class="block text-xs font-bold uppercase tracking-wider" style="color: var(--muted-foreground);">Brand
                            Name</label>
                        <input type="text" id="modal_name" name="name" x-model="form.name" required
                            placeholder="e.g. Corsair, G.Skill"
                            class="w-full p-3 rounded-xl focus:ring-1 focus:ring-ring focus:border-ring focus:ring-4 focus:ring-ring/15 focus:outline-none transition-all" style="background: var(--muted); border: 1px solid var(--border); color: var(--foreground);">
                    </div>

                    <!-- Brand Logo -->
                    <div class="space-y-1.5">
                        <label class="block text-xs font-bold uppercase tracking-wider" style="color: var(--muted-foreground);">Brand Logo
                            Image</label>
                        ...
                        <div class="flex items-center space-x-4">
                            ...
                            <div class="flex-1 space-y-2">
                                ...
                                <p class="text-[10px]" style="color: var(--muted-foreground);">PNG or SVG format with transparent or white background
                                    is ideal.</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        <label class="block text-xs font-bold uppercase tracking-wider" style="color: var(--muted-foreground);">Map to
                            Categories</label>
                        <div
                            class="grid grid-cols-2 gap-2 rounded-xl p-3.5 max-h-48 overflow-y-auto" style="background: var(--muted); border: 1px solid var(--border);">
                            @foreach($categories as $cat)
                                <label class="flex items-center space-x-2 py-1 cursor-pointer">
                                    ...
                                    <span class="text-xs font-medium" style="color: var(--foreground);">{{ $cat->name }}</span>
                                </label>
                            @endforeach
                        </div>
                    </div>
