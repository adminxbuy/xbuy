...
<div x-data="{ 
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-1 p-1 bg-muted rounded-lg w-fit">
            <button @click="tab = 'images'; selectedFiles = []" 
                    :class="tab === 'images' ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'"
            ...
            <button @click="tab = 'pdfs'; selectedFiles = []"
                    :class="tab === 'pdfs' ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'"
                    class="px-4 py-2 text-xs font-medium rounded-md transition-all flex items-center gap-2">
            ...
            <button @click="tab = 'videos'; selectedFiles = []"
                    :class="tab === 'videos' ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'"
                    class="px-4 py-2 text-xs font-medium rounded-md transition-all flex items-center gap-2">
        ...
        <div class="flex items-center gap-2">
            <div class="relative">
                ...
                <input type="text" x-model="searchQuery" placeholder="Search files..." 
                       class="h-9 pl-9 pr-4 rounded-lg border border-input bg-transparent px-3 py-1 text-sm shadow-xs focus:border-ring focus:ring-2 focus:ring-ring/50 outline-none w-48">
            </div>
            <div class="flex items-center gap-1 p-1 bg-muted rounded-lg">
                <button @click="viewMode = 'grid'" :class="viewMode === 'grid' ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground'" class="p-1.5 rounded-md transition-all">
                    <i data-lucide="grid-3x3" class="w-4 h-4"></i>
                </button>
                <button @click="viewMode = 'list'" :class="viewMode === 'list' ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground'" class="p-1.5 rounded-md transition-all">
            ...
            <form action="{{ route('admin.content.upload') }}" method="POST" enctype="multipart/form-data" id="upload-form">
                ...
                <label class="inline-flex items-center gap-2 h-9 px-4 rounded-lg text-xs font-medium bg-primary text-primary-foreground hover:bg-primary/90 shadow-sm cursor-pointer transition-colors">
    ...
    <div x-show="selectedFiles.length > 0" x-cloak
         ...
         class="flex items-center gap-3 p-3 rounded-xl border border-border bg-card shadow-sm">
                                        ...
                                        <div class="flex items-center gap-1.5 pt-2 border-t border-border">
                                            <button type="button" @click="copyToClipboard('{{ $file['url'] }}', '{{ $file['name'] }}')"
                                                ...
                                                <template x-if="copiedName === '{{ $file['name'] }}'">
                                                    <span class="flex items-center gap-1 text-emerald-600"><i data-lucide="check" class="w-3 h-3"></i> Copied</span>
                                                </template>
                                                <template x-if="copiedName !== '{{ $file['name'] }}'">
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div class="w-full max-w-sm rounded-xl border border-border bg-background p-6 shadow-lg" @click.outside="confirmDelete = false"
                ...
                <form action="{{ route('admin.content.delete') }}" method="POST" class="flex gap-2 mt-6">
                    ...
                    <button type="submit" class="flex-1 h-9 inline-flex items-center justify-center rounded-lg text-xs font-medium bg-destructive text-destructive-foreground hover:bg-destructive/90 shadow-sm transition-colors">
                        Delete
                    </button>
                </form>
            </div>
