...
<div class="space-y-6" x-data="{ 
    ...
    <!-- Header Summary -->
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-zinc-50 border border-zinc-200 rounded-xl p-5">
        <div>
            <p class="text-xs text-zinc-400 uppercase tracking-wider font-bold mb-1">Public Assets Directory</p>
            <p class="text-sm text-zinc-550 leading-relaxed">
    ...
    <div class="flex border-b border-zinc-200 overflow-x-auto pb-1 gap-1">
        <button type="button" @click="tab = 'images'" 
                :class="tab === 'images' ? 'border-[#09090b] font-semibold text-black' : 'border-transparent text-zinc-500 hover:text-zinc-800'" 
            ...
            <span>Images</span>
            <span class="ml-1 text-[10px] px-1.5 py-0.5 bg-zinc-100 border border-zinc-200 text-zinc-550 rounded-full font-bold">
                {{ count($content['images']) }}
            </span>
        </button>
        <button type="button" @click="tab = 'pdfs'" 
                :class="tab === 'pdfs' ? 'border-[#09090b] font-semibold text-black' : 'border-transparent text-zinc-500 hover:text-zinc-800'" 
                class="px-4 py-3 border-b-2 text-xs uppercase tracking-wider transition-all focus:outline-none flex items-center space-x-2 whitespace-nowrap">
        ...
        <button type="button" @click="tab = 'videos'" 
                :class="tab === 'videos' ? 'border-[#09090b] font-semibold text-black' : 'border-transparent text-zinc-500 hover:text-zinc-800'" 
                class="px-4 py-3 border-b-2 text-xs uppercase tracking-wider transition-all focus:outline-none flex items-center space-x-2 whitespace-nowrap">
    ...
    <div class="bg-white border border-zinc-200 rounded-xl p-6 shadow-sm">
        ...
        <form action="{{ route('admin.content.upload') }}" method="POST" enctype="multipart/form-data" class="mb-8 p-5 bg-zinc-50 border border-zinc-200 rounded-xl space-y-4">
            @csrf
            <input type="hidden" name="type" :value="tab">
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                <div class="space-y-1">
                    <h4 class="font-bold text-zinc-800 text-xs uppercase tracking-wider flex items-center">
                        <i data-lucide="upload-cloud" class="w-4 h-4 mr-2 text-zinc-650"></i>
                        Upload to Category: <span class="text-[#ffd747] capitalize ml-1" x-text="tab"></span>
                    </h4>
                    <p class="text-[10px] text-zinc-400">File size must be under 20MB.</p>
                </div>

                <div class="flex items-center space-x-3 w-full sm:w-auto">
                    <label class="flex-1 sm:flex-none px-4 py-2 border border-zinc-200 bg-white hover:bg-zinc-50 text-zinc-700 rounded-xl text-xs font-semibold cursor-pointer transition-all flex items-center justify-center space-x-1.5 shadow-sm">
        ...
        @foreach(['images', 'pdfs', 'videos'] as $cType)
            <div x-show="tab === '{{ $cType }}'" x-cloak>
                @if(count($content[$cType]) === 0)
                    <div class="text-center py-16 text-zinc-400 border-2 border-dashed border-zinc-200 rounded-xl">
                        <i data-lucide="folder-x" class="w-12 h-12 mx-auto mb-3 text-zinc-300"></i>
                        <p class="text-sm font-semibold">No assets found in <span class="capitalize">{{ $cType }}</span></p>
                        <p class="text-[11px] text-zinc-400 mt-1">Upload a file above to add content.</p>
                    </div>
                @else
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-6">
                        @foreach($content[$cType] as $file)
                            <div class="group bg-white border border-zinc-200 hover:border-zinc-300 rounded-xl overflow-hidden shadow-sm flex flex-col justify-between transition-all hover:shadow-md">
                                <!-- Preview Header -->
                                <div class="bg-zinc-50 flex items-center justify-center p-4 border-b border-zinc-150 h-36 overflow-hidden relative">
                                    @if($cType === 'images')
                                ...
                                <div class="p-3.5 space-y-2.5 flex-1 flex flex-col justify-between">
                                    <div>
                                        <h5 class="text-xs font-semibold text-zinc-850 truncate" title="{{ $file['name'] }}">{{ $file['name'] }}</h5>
                                        <div class="flex items-center justify-between text-[9px] text-zinc-400 mt-1.5">
                                            <span class="font-medium bg-zinc-100 border border-zinc-200 px-1.5 py-0.5 rounded-full" x-text="'{{ $file['size'] }}'"></span>
                                            <span>{{ date('d M Y, H:i', strtotime($file['updated_at'])) }}</span>
                                        </div>
                                    </div>

                                    <div class="flex items-center gap-2 pt-2.5 border-t border-zinc-100">
                                        <!-- Copy URL Button -->
                                        <button type="button" @click="copyToClipboard('{{ $file['url'] }}', '{{ $file['name'] }}')"
                                                class="flex-1 py-1.5 border border-zinc-200 hover:border-zinc-300 text-zinc-700 bg-white hover:bg-zinc-50 rounded-lg text-[10px] font-bold transition-all flex items-center justify-center space-x-1 shadow-sm">
    ...
    <div x-show="confirmDelete" x-transition x-cloak class="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
        <div class="bg-white rounded-xl max-w-sm w-full shadow-2xl border border-zinc-200" @click.away="confirmDelete = false">
            <div class="p-6 text-center space-y-4">
                ...
                <div class="space-y-1.5">
                    <h3 class="text-sm font-bold text-zinc-900 uppercase tracking-wider">Confirm File Deletion</h3>
            ...
            <form action="{{ route('admin.content.delete') }}" method="POST" class="p-4 border-t border-zinc-150 flex justify-end space-x-2 bg-zinc-50 rounded-b-2xl">
                @csrf
                @method('DELETE')
                <input type="hidden" name="path" :value="deletePath">
                
                <button type="button" @click="confirmDelete = false" class="px-4 py-2 border border-zinc-200 bg-white hover:bg-zinc-50 text-zinc-700 rounded-xl text-xs font-semibold shadow-sm transition-all">
                    Cancel
                </button>
                <button type="submit" class="px-5 py-2 bg-red-600 hover:bg-red-750 text-white rounded-xl text-xs font-semibold shadow-sm border border-black/10 transition-all active:scale-[0.98]">
                    Delete File
                </button>
            </form>
        </div>
