...
@section('header_actions')
 <a href="{{ route('admin.trash.index', 'disputes') }}" class="flex items-center space-x-1.5 px-3 py-1.5 bg-muted hover:bg-muted text-foreground rounded-lg text-xs font-semibold transition-all border border-border/50">
...
<div class="rounded-xl overflow-hidden mb-6 bg-card text-card-foreground border border-border" x-data="{ selectedIds: [], selectAll: false, bulkAction: '' }">
 ...
 @if($dispute->urgency_status === 'critical') bg-muted text-foreground border-border/60
 @elseif($dispute->urgency_status === 'high') bg-muted text-foreground border-border/60
 ...
 @endif
  ...
  <!-- Status badge -->
 <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium border w-max
 @if($dispute->status === 'open') bg-muted text-amber-750 border-border/60
 ...
 @elseif($dispute->status === 'resolved') bg-muted text-emerald-750 border-border/60
