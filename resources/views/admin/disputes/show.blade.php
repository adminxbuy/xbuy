...
@section('content')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 flex flex-col space-y-6">
            ...
            <div class="rounded-xl p-6 shadow-sm bg-card text-card-foreground border border-border">
                    ...
                    <div class="flex items-center gap-2">
                        @if($dispute->status === 'resolved')
                            <form action="{{ route('admin.disputes.reopen', $dispute->id) }}" method="POST">
                                @csrf
                                <button type="submit"
                                    class="inline-flex items-center gap-2 font-semibold px-4 py-2 rounded-lg text-sm transition-all shadow-sm bg-primary text-primary-foreground">
                ...
                <p class="text-xs text-muted-foreground">Order: <span
                    ...
                    <span class="px-2 py-0.5 text-xs rounded-full font-semibold border inline-block
                        @if($dispute->status === 'open') bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950/60 dark:text-amber-300 dark:border-amber-900
                        @elseif($dispute->status === 'seller_responded') bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-950/60 dark:text-blue-300 dark:border-blue-900
                        @elseif($dispute->status === 'under_review') bg-muted text-foreground border-border
                        @elseif($dispute->status === 'resolved') bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-950/60 dark:text-emerald-300 dark:border-emerald-900
                ...
                <div class="mt-6 p-4 bg-rose-50 border border-rose-200 dark:bg-rose-950/60 dark:border-rose-900 rounded-xl">
                    <span class="text-xs font-semibold text-rose-700 dark:text-rose-300 block uppercase mb-2">Buyer Complaint</span>
            ...
            @if($dispute->responses->where('user_id', $dispute->seller_id)->count() > 0)
                <div class="rounded-xl p-6 shadow-sm bg-card text-card-foreground border border-border">
                    ...
                    <div class="space-y-4 mb-4 p-4 bg-blue-50 border border-blue-200 dark:bg-blue-950/60 dark:border-blue-900 rounded-xl">
                        @foreach($dispute->responses->where('user_id', $dispute->seller_id) as $response)
                            <div>
                                <p class="text-xs font-semibold text-blue-700 dark:text-blue-300 mb-2">{{ $dispute->seller->shop_name }} •
                                ...
                                @if($response->evidence_images && count($response->evidence_images) > 0)
                                    <div class="mt-3 grid grid-cols-2 sm:grid-cols-3 gap-3">
                                        @foreach($response->evidence_images as $url)
                                            <button type="button" onclick="openLightbox('{{ $url }}')"
                                                class="group block aspect-video rounded-lg overflow-hidden border border-blue-200 bg-blue-50 dark:border-blue-900 dark:bg-blue-950/60 hover:border-ring transition-all cursor-pointer">
            ...
            <div class="rounded-lg flex-1 flex flex-col overflow-hidden bg-card text-card-foreground border border-border">
                ...
                <div class="p-6 space-y-6 overflow-y-auto max-h-[400px]">
                    @if($dispute->status === 'resolved')
                        <div
                            class="p-4 bg-emerald-50 border border-emerald-200 text-emerald-700 dark:bg-emerald-950/60 dark:border-emerald-900 dark:text-emerald-300 rounded-xl text-sm font-semibold mb-6 flex items-start gap-3 shadow-sm">
                            <i data-lucide="check-circle" class="w-5 h-5 text-emerald-600 dark:text-emerald-400 shrink-0 mt-0.5"></i>
                            <div>
                                <p class="font-bold text-emerald-800 dark:text-emerald-200">Dispute Resolved by Admin</p>
                                <p class="text-xs text-emerald-700 dark:text-emerald-300 mt-1">Decision: <span
                                        class="font-bold uppercase">{{ str_replace('_', ' ', $dispute->admin_decision) }}</span>
                                </p>
                                <p class="text-xs text-emerald-700 dark:text-emerald-300">Fault Assigned: <span
                                        class="font-bold uppercase">{{ $dispute->fault_assigned_to }}</span></p>
                                @if($dispute->buyer_payout > 0 || $dispute->seller_payout > 0)
                                    <p class="text-xs text-emerald-700 dark:text-emerald-300">Payouts: Buyer:
                                        ₹{{ number_format($dispute->buyer_payout, 2) }} | Seller:
                                        ₹{{ number_format($dispute->seller_payout, 2) }}</p>
                                @endif
                                @if($dispute->resolution_notes)
                                    <p
                                        class="text-xs text-emerald-700 dark:text-emerald-300 mt-2 bg-card/60 p-2.5 rounded-xl border border-emerald-200 dark:border-emerald-900 italic">
        ...
        <div class="space-y-6">
            ...
            <div class="rounded-xl p-6 shadow-sm bg-card text-card-foreground border border-border">
                ...
                <div class="space-y-3.5 text-xs">
                    ...
                    <div class="flex justify-between items-center py-1.5">
                        ...
                        <span class="font-bold text-amber-700 bg-amber-50 border border-amber-200 dark:bg-amber-950/60 dark:text-amber-300 dark:border-amber-900 px-2 py-0.5 rounded-full text-[10px]">
            ...
            <div class="rounded-xl p-6 shadow-sm bg-card text-card-foreground border border-border">
                ...
                <div class="space-y-3">
                    <div class="p-3 bg-rose-50 border border-rose-200 dark:bg-rose-950/60 dark:border-rose-900 rounded-lg">
                        <p class="text-[10px] font-semibold text-rose-700 dark:text-rose-300 mb-1">Buyer</p>
                        <p class="text-xs font-semibold text-foreground">{{ $dispute->buyer->name }}</p>
                    </div>
                    <div class="p-3 bg-blue-50 border border-blue-200 dark:bg-blue-950/60 dark:border-blue-900 rounded-lg">
                        <p class="text-[10px] font-semibold text-blue-700 dark:text-blue-300 mb-1">Seller</p>
        ...
        <div class="w-full max-w-2xl shadow-2xl flex flex-col overflow-hidden bg-card text-card-foreground">
            ...
            <div class="flex-1 overflow-y-auto">
                ...
                @if($dispute->status === 'resolved')
                    ...
                    <div class="p-6">
                        <div
                            class="p-4 bg-emerald-50 border border-emerald-200 text-emerald-700 dark:bg-emerald-950/60 dark:border-emerald-900 dark:text-emerald-300 rounded-xl text-sm font-semibold">
                            <p class="font-bold text-emerald-800 dark:text-emerald-200 mb-2 flex items-center gap-2">
                                <i data-lucide="check-circle" class="w-5 h-5"></i>
                                This dispute has already been resolved
                            </p>
                            <p class="text-emerald-700 dark:text-emerald-300 mb-3">Decision: <span
                                    class="font-bold">{{ ucfirst(str_replace('_', ' ', $dispute->admin_decision)) }}</span></p>
                            <p class="text-emerald-700 dark:text-emerald-300 mb-3">Fault: <span
                                    class="font-bold">{{ ucfirst($dispute->fault_assigned_to) }}</span></p>
                            @if($dispute->resolution_notes)
                                <p class="text-emerald-700 dark:text-emerald-300 mt-3 p-2 bg-card/50 rounded border border-emerald-200 dark:border-emerald-900">
                                    {{ $dispute->resolution_notes }}</p>
                            @endif
                            <form action="{{ route('admin.disputes.reopen', $dispute->id) }}" method="POST"
                                class="mt-4 pt-4 border-t border-emerald-200 dark:border-emerald-900">
                                @csrf
                                <button type="submit"
                    ...
                    <form action="{{ route('admin.disputes.resolve', $dispute->id) }}" method="POST" class="p-6 space-y-6">
                        ...
                        <div id="payoutPreview" class="p-4 rounded-lg space-y-2 hidden bg-muted border border-border">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div class="p-3 rounded-lg border border-rose-200 dark:border-rose-900 bg-card">
                                    <p class="text-[10px] font-semibold text-muted-foreground">Refund to Buyer</p>
                                    <p id="buyerPayoutPreview" class="text-base font-bold text-rose-600 dark:text-rose-400">₹0.00</p>
                                </div>
                                <div class="p-3 rounded-lg border border-emerald-200 dark:border-emerald-900 bg-card">
                                    <p class="text-[10px] font-semibold text-muted-foreground">Payout to Seller</p>
                                    <p id="sellerPayoutPreview" class="text-base font-bold text-emerald-600 dark:text-emerald-400">₹0.00</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <!-- Submit Button -->
                        <button type="submit"
                            class="w-full font-semibold py-2.5 px-4 rounded-lg shadow-sm transition-all flex items-center justify-center space-x-2 text-xs h-10 bg-primary text-primary-foreground">
