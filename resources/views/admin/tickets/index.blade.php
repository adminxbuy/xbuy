...
@section('header_actions')
 <a href="{{ route('admin.trash.index', 'tickets') }}" class="flex items-center space-x-1.5 px-4 py-2.5 bg-muted hover:bg-muted text-foreground rounded-lg border border-border/50 text-xs font-medium transition-all border border-border ">
...
<div class="bg-card border border-border rounded-xl ring-0 mb-6" x-data="{ selectedIds: [], selectAll: false, bulkAction: '' }">
 ...
 <a href="{{ route('admin.tickets') }}"  class="px-4 py-2.5 border border-border text-muted-foreground hover:bg-muted rounded-lg text-xs font-semibold flex items-center transition-all">
 Clear Filters
 </a>
 @endif
 </div>
 ...
 <!-- Bulk Action Toolbar -->
 <div x-show="selectedIds.length > 0" x-transition.opacity style="display: none;" class="bg-muted border-y border-border p-3 flex items-center justify-between">
 <div class="flex items-center space-x-3">
 <span class="text-foreground font-bold text-xs uppercase tracking-wider bg-muted/50 px-3 py-1 rounded-lg"><span x-text="selectedIds.length"></span> Selected</span>
 ...
 <select name="action" x-model="bulkAction" class="text-xs border-border/50 bg-card rounded-lg focus:ring-amber-500 focus:border-amber-500 py-1.5 px-3 font-semibold text-foreground">
 ...
 <button type="button" @click="selectedIds = []; selectAll = false" class="text-muted-foreground hover:text-foreground text-xs font-bold px-2 py-1 uppercase tracking-wider">Clear</button>
 </div>
 ...
 @if($ticket->status === 'open') bg-muted text-foreground border border-border
 @elseif($ticket->status === 'in_progress') bg-muted text-foreground border border-border
 ...
 @csrf
 <select name="status" onchange="this.form.submit()"  class="p-1.5 pr-8 border border-border rounded-lg text-xs font-semibold text-foreground bg-card hover:bg-muted focus:outline-none focus:ring-1 focus:ring-ring focus:ring-1 focus:ring-ring focus:border-ring">
