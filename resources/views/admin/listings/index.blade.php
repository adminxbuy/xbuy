...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-5">
 ...
 <span class="text-[10px] font-bold {{ $c['up'] ? 'text-muted-foreground' : 'text-muted-foreground' }}">{{ $c['change'] }}</span>
 </div>
...
<div class="bg-card border border-border rounded-xl overflow-hidden">
 ...
 <span class="text-[9px] font-bold bg-muted text-foreground px-1.5 py-0.5 rounded ml-1">⚠ Prev. Sold</span>
 @endif
 </div>
 </div>
 </td>
 ...
 'active' => ['text-foreground bg-muted border-border', 'Active'],
 'expired' => ['text-muted-foreground bg-muted border-border', 'Expired'],
 default => ['text-muted-foreground bg-muted border-border', 'None'],
 };
 @endphp
 ...
 'full' => ['text-foreground bg-muted border-border', 'Full'],
 ...
 <button type="submit"
 class="w-full flex items-center gap-2.5 px-3 py-2.5 text-xs text-muted-foreground hover:bg-muted transition-colors text-left">
...
<div x-data="{ open: false }" class="mt-4 bg-card border border-border rounded-xl overflow-hidden">
 ...
 @csrf
 <button type="submit" class="inline-flex items-center gap-1 px-2.5 py-1.5 border border-border bg-muted hover:bg-muted text-foreground rounded-lg text-[10px] font-semibold transition-all">
 ...
 @csrf @method('DELETE')
 <button type="submit" class="inline-flex items-center gap-1 px-2.5 py-1.5 border border-border bg-muted hover:bg-muted text-muted-foreground rounded-lg text-[10px] font-semibold transition-all">
...
<div x-show="drawerOpen"
 ...
 'bg-muted text-foreground border-border': listing?.listing_status === 'active',
 'bg-muted text-foreground border-border': listing?.listing_status === 'paused',
 'bg-muted text-foreground border-border': listing?.listing_status === 'rejected',
 'bg-muted text-foreground border-border': listing?.listing_status === 'sold',
 ...
 class="mx-6 mt-5 flex items-start gap-3 p-3.5 bg-muted border border-border rounded-xl">
 <i data-lucide="alert-triangle" class="w-4 h-4 text-muted-foreground shrink-0 mt-0.5"></i>
 <p class="text-xs text-foreground font-semibold leading-relaxed">Previously sold serial number — verify authenticity carefully before approving.</p>
 </div>
 ...
 <span class="text-xs font-bold font-mono text-foreground select-all" x-text="listing?.listing_number || '—'"></span>
 </div>
 <div class="flex items-center justify-between px-4 py-3 bg-muted border border-border rounded-xl">
 ...
 :class="{ 'text-foreground': listing?.manufacturer_warranty_status==='active', 'text-muted-foreground': listing?.manufacturer_warranty_status==='expired', 'text-muted-foreground': listing?.manufacturer_warranty_status==='none' }"
 ...
 :class="{ 'text-foreground': listing?.document_status==='full', 'text-foreground': listing?.document_status==='partial', 'text-muted-foreground': listing?.document_status==='none' }"
 ...
 {{-- Rejection Reason --}}
 <div x-show="listing?.rejection_reason" class="mt-3 p-4 bg-muted border border-border rounded-xl">
 <p class="text-[10px] font-bold text-muted-foreground uppercase tracking-wide mb-1">Rejection Reason</p>
 <p class="text-xs text-foreground leading-relaxed" x-text="listing?.rejection_reason"></p>
 </div>
 </div>
 </div>
