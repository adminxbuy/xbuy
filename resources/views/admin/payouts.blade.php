        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div style="background: var(--card); border: 1px solid var(--border);" class="rounded-xl p-5 shadow-sm flex flex-col gap-2">
                <div class="p-2 bg-emerald-50 text-emerald-700 rounded-xl w-fit"><i data-lucide="wallet"
                        class="w-5 h-5"></i></div>
                <h3 class="text-2xl font-bold" style="color: var(--foreground);">₹{{ number_format($releasedThisMonth, 2) }}</h3>
                <p class="text-[10px] font-bold uppercase tracking-wider" style="color: var(--muted-foreground);">Released This Month</p>
            </div>

            <!-- Card 2: Total Commission Earned -->
            <div style="background: var(--card); border: 1px solid var(--border);" class="rounded-xl p-5 shadow-sm flex flex-col gap-2">
                <div class="p-2 rounded-xl w-fit" style="background: var(--primary); color: var(--primary-foreground);"><i data-lucide="percent"
                        class="w-5 h-5"></i></div>
                <h3 class="text-2xl font-bold" style="color: var(--foreground);">₹{{ number_format($totalCommission, 2) }}</h3>
                <p class="text-[10px] font-bold uppercase tracking-wider" style="color: var(--muted-foreground);">Total Commission Earned</p>
            </div>
            ...
            <div style="background: var(--card); border: 1px solid var(--border);" class="rounded-xl p-5 shadow-sm flex flex-col gap-2">
                ...
                <h3 class="text-2xl font-bold" style="color: var(--foreground);">₹{{ number_format($avgPayout, 2) }}</h3>
                <p class="text-[10px] font-bold uppercase tracking-wider" style="color: var(--muted-foreground);">Average Payout Amount</p>
            </div>
        </div>

        <!-- Filters & Export -->
        <div style="background: var(--card); border: 1px solid var(--border);"
            class="rounded-xl p-5 shadow-sm flex flex-col md:flex-row justify-between items-stretch md:items-end gap-4">
            <form method="GET" action="{{ route('admin.payouts') }}"
                ...
                <div class="flex flex-wrap items-center gap-4 flex-1">
                    ...
                    <div class="date-range-picker-container flex flex-wrap items-center gap-4" x-data="dateRangePicker({
                        ...
                        <div class="space-y-1">
                            <label class="text-xs font-bold uppercase tracking-wider block" style="color: var(--muted-foreground);">Released Date
                                Range</label>
                            <div class="relative">
                                ...
                                <div
                                    class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2" style="color: var(--muted-foreground);">
                ...
                <div class="flex items-center gap-2 pt-5">
                    @if(request()->anyFilled(['seller', 'date_start', 'date_end']))
                        <a href="{{ route('admin.payouts') }}"
                            class="px-4 py-2 rounded-xl text-xs font-semibold flex items-center transition-all" style="border: 1px solid var(--border); color: var(--muted-foreground);">
                            Clear Filters
                        </a>
                    @endif
                </div>
            ...
            <div class="border-t md:border-t-0 md:border-l pt-4 md:pt-0 md:pl-4 flex items-end" style="border-color: var(--border);">
                <a href="{{ route('admin.payouts.export', request()->query()) }}"
                    class="w-full md:w-auto font-semibold py-2.5 px-5 rounded-lg ring-0 transition-all flex items-center justify-center space-x-2 text-xs"
                ...
                <div class="flex items-center space-x-3">
                    ...
                    <form method="POST" action="{{ route('admin.payouts.bulk-action') }}"
                        ...
                        <select name="action" x-model="bulkAction"
                            class="text-xs border-amber-200/50 rounded-lg focus:ring-amber-500 focus:border-amber-500 py-1.5 px-3 font-semibold" style="background: var(--card); color: var(--foreground);">
                    ...
                    <tbody class="divide-y" style="border-color: var(--border);">
                        @forelse($payouts as $payout)
                            ...
                            <tr class="hover:bg-muted transition-colors" style="color: var(--muted-foreground);">
                                ...
                                <!-- Order # -->
                                <td class="p-4 font-bold text-xs" style="color: var(--foreground);">
                                    #{{ $payout->order->order_number ?? 'N/A' }}
                                </td>
                                <!-- Seller Name -->
                                <td class="p-4 font-semibold text-xs" style="color: var(--foreground);">
                                    {{ $payout->order->seller->shop_name ?? 'N/A' }}
                                </td>
                                <!-- Gross Amount -->
                                <td class="p-4 text-right font-semibold" style="color: var(--foreground);">
                        ...
                        @empty
                            <tr>
                                <td colspan="11" class="p-12 text-center italic" style="color: var(--muted-foreground);">
