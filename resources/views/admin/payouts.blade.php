...
@section('content')
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div class="rounded-xl p-5 shadow-sm flex flex-col gap-2 bg-card border border-border">
                <div class="p-2 bg-emerald-50 text-emerald-700 rounded-xl w-fit dark:bg-emerald-950/60 dark:text-emerald-300"><i data-lucide="wallet"
                        class="w-5 h-5"></i></div>
            ...
            <div class="rounded-xl p-5 shadow-sm flex flex-col gap-2 bg-card border border-border">
                <div class="p-2 bg-blue-50 text-blue-700 rounded-xl w-fit dark:bg-blue-950/60 dark:text-blue-300"><i data-lucide="bar-chart-2" class="w-5 h-5"></i>
                </div>
        ...
        <!-- Filters & Export -->
        <div class="rounded-xl p-5 shadow-sm flex flex-col md:flex-row justify-between items-stretch md:items-end gap-4 bg-card border border-border">
        ...
        <div class="rounded-xl ring-0 overflow-hidden bg-card border border-border"
            ...
            <div x-show="selectedIds.length > 0" x-transition.opacity x-cloak
                class="bg-amber-50 border-b border-amber-100 p-3 flex items-center justify-between dark:bg-amber-950/60 dark:border-amber-900">
                <div class="flex items-center space-x-3">
                    ...
                    <form method="POST" action="{{ route('admin.payouts.bulk-action') }}"
                        ...
                        <select name="action" x-model="bulkAction"
                            class="text-xs border-amber-200/50 rounded-lg focus:ring-amber-500 focus:border-amber-500 py-1.5 px-3 font-semibold bg-card text-foreground">
                            ...
                            <tr class="hover:bg-muted transition-colors text-muted-foreground">
                                ...
                                <td class="p-4 pr-6 text-center">
                                    <span class="px-2 py-0.5 text-[9px] rounded-full font-bold uppercase tracking-wider border
                                            @if($payout->status === 'released') bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-950/60 dark:text-emerald-300 dark:border-emerald-900
                                            @elseif($payout->status === 'partially_released') bg-purple-100 text-purple-800 border-purple-200 dark:bg-purple-950/60 dark:text-purple-300 dark:border-purple-900
