        ...
        <form action="{{ route('admin.ratings.store') }}" method="POST" class="space-y-6">
            ...
            <div class="space-y-1.5">
                ...
                <div class="relative">
                    <select name="order_id" id="order_id" required
                            class="w-full p-3 pl-4 pr-10 text-xs border border-border rounded-xl bg-muted hover:bg-muted focus:bg-card focus:ring-1 focus:ring-ring focus:border-ring focus:outline-none transition-all appearance-none cursor-pointer @error('order_id') border-red-500 @enderror">
                ...
                @enderror
                <p class="text-[10px] text-muted-foreground mt-1">Note: Only orders that do not currently have a rating record are listed here.</p>
            </div>
                ...
                <div class="bg-muted border border-border rounded-xl p-4 flex items-center justify-between">
                    <div>
                        ...
                        <div class="flex items-baseline gap-1.5">
                            <span class="text-2xl font-bold text-foreground" x-text="average">5.00</span>
            ...
            <div class="space-y-1.5 pt-2">
                ...
                <textarea name="review_text" id="review_text" rows="4" placeholder="Enter comments or reason for override here..."
                          class="w-full p-3 text-xs border border-border rounded-xl bg-muted focus:bg-card focus:ring-1 focus:ring-ring focus:border-ring focus:outline-none transition-all @error('review_text') border-red-500 @enderror">{{ old('review_text') }}</textarea>
                @error('review_text')
                    <p class="text-red-500 text-[10px] font-semibold mt-1">{{ $message }}</p>
                @enderror
            </div>
