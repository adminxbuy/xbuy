...
@section('header_actions')
    <a href="{{ route('admin.trash.index', 'orders') }}" class="flex items-center space-x-1.5 px-4 py-2 rounded-lg text-xs font-bold transition-all border bg-rose-50 text-rose-700 border-rose-200 dark:bg-rose-950/60 dark:text-rose-300 dark:border-rose-900">
                            ...
                            <td class="px-5 py-3.5">
                                @if($order->escrow)
                                    <span class="text-[10px] font-bold px-2 py-0.5 rounded-full border
                                        @if($order->escrow->status === 'held') bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950/60 dark:text-amber-300 dark:border-amber-900
                                        @elseif($order->escrow->status === 'released') bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-950/60 dark:text-emerald-300 dark:border-emerald-900
                                        @elseif($order->escrow->status === 'disputed') bg-rose-50 text-rose-700 border-rose-200 dark:bg-rose-950/60 dark:text-rose-300 dark:border-rose-900
    ...
    <div x-show="drawerOpen" x-transition:enter="transition ease-out duration-300 transform" x-transition:enter-start="translate-x-full" x-transition:enter-end="translate-x-0" x-transition:leave="transition ease-in duration-200 transform" x-transition:leave-start="translate-x-0" x-transition:leave-end="translate-x-full" class="fixed top-0 right-0 h-full w-full max-w-xl shadow-2xl z-50 flex flex-col bg-card" style="display:none;">
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-6">
            <template x-if="order?.dispute_id">
                <div class="p-3.5 rounded-xl flex items-center justify-between gap-3 border bg-rose-50 border-rose-200 dark:bg-rose-950/60 dark:border-rose-900">
                    <div class="flex items-center gap-2"><i data-lucide="alert-triangle" class="w-4 h-4 shrink-0 text-rose-500 dark:text-rose-400"></i><p class="text-xs font-bold text-rose-700 dark:text-rose-300">Active dispute on this order.</p></div>
                    <a :href="order?.dispute_url" class="text-xs font-bold px-3 py-1 rounded-lg transition-all shrink-0 bg-rose-100 text-rose-700 dark:bg-rose-900 dark:text-rose-200">View Dispute</a>
                </div>
            </template>
            ...
            <div>
                ...
                <div class="rounded-xl p-4 flex items-center gap-3 bg-muted border border-border">
                    ...
                    <div class="text-right shrink-0"><p class="text-[10px] text-muted-foreground">Payout</p><p class="text-sm font-bold text-emerald-600 dark:text-emerald-400" x-text="'₹' + order?.seller_payout"></p></div>
            ...
            <div>
                ...
                <div class="rounded-xl p-4 space-y-3 bg-muted border border-border">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-[10px] font-bold px-2 py-0.5 rounded-full" :class="{ 'bg-amber-50 text-amber-700 border border-amber-200 dark:bg-amber-950/60 dark:text-amber-300 dark:border-amber-900': order?.escrow_status === 'held', 'bg-emerald-50 text-emerald-700 border border-emerald-200 dark:bg-emerald-950/60 dark:text-emerald-300 dark:border-emerald-900': order?.escrow_status === 'released', 'bg-rose-50 text-rose-700 border border-rose-200 dark:bg-rose-950/60 dark:text-rose-300 dark:border-rose-900': order?.escrow_status === 'disputed' }" x-text="order?.escrow_status === 'none' ? 'No Escrow' : order?.escrow_status?.charAt(0).toUpperCase() + order?.escrow_status?.slice(1)"></span>
                    </div>
            ...
            <template x-if="order?.awb_number">
                <div>
                    ...
                    <div class="rounded-xl p-4 space-y-2 text-xs bg-muted border border-border">
                        ...
                        <div class="flex justify-between" x-show="order?.delivered_at"><span class="text-muted-foreground">Delivered</span><span class="font-medium text-emerald-600 dark:text-emerald-400" x-text="order?.delivered_at"></span></div>
