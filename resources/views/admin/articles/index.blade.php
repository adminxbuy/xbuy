...
<div x-data="{ selectedIds: [], selectAll: false, bulkAction: '' }" class="space-y-4">
 ...
 <h3 class="text-2xl font-bold text-foreground">{{ \App\Models\Article::where('is_published', true)->count() }}</h3>
 </div>
 </div>
 <div class="bg-muted border border-border rounded-xl p-5 flex items-center space-x-4">
 <div class="p-3 bg-muted text-foreground border border-border/60 rounded-xl">
 ...
 <input type="text" name="search" value="{{ request('search') }}" placeholder="Search title, excerpt or content..."  class="w-full pl-9 pr-4 py-2 border border-border rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-ring focus:border-ring focus:border-transparent">
 ...
 class="bg-muted border border-border rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 ">
 ...
 <button type="submit" :disabled="!bulkAction" class="bg-primary text-primary-foreground hover:bg-primary/90 disabled:opacity-50 text-primary-foreground font-medium px-4 py-1.5 rounded-lg text-xs transition-all ">
 Apply
 </button>
 ...
 <td class="py-4 px-5" x-data="{  isPublished: {{ $article->is_published ? 'true' : 'false' }},  async toggle() {
 try {
 ...
 <span class="w-9 h-5 rounded-full p-0.5 transition-colors duration-200 ease-in-out shrink-0"  :class="isPublished ? 'bg-emerald-500' : 'bg-muted'">
 ...
 <span class="text-xs font-semibold" :class="isPublished ? 'text-foreground' : 'text-muted-foreground'" x-text="isPublished ? 'Published' : 'Draft'"></span>
 </button>
 </td>
 <td class="py-4 px-5">
 ...
 @csrf
 @method('DELETE')
 <button type="submit" class="p-1.5 text-muted-foreground hover:text-muted-foreground hover:bg-muted rounded-lg transition-all" title="Delete Article">
