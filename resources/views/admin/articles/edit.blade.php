...
@section('content')
<div class="max-w-6xl mx-auto" x-data="{  title: '{{ addslashes($article->title) }}',  slug: '{{ $article->slug }}',  generateSlug() {  this.slug = this.title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');  },
 activeTab: 'write'
}">
 <!-- Back Button -->
 <div class="mb-6">
 ...
 <div class="space-y-6">
 <!-- Cover Image Block -->
 <div class="bg-card border border-border rounded-xl p-6 space-y-4 " x-data="{ imageType: '{{ filter_var($article->cover_image, FILTER_VALIDATE_URL) ? 'url' : 'file' }}' }">
