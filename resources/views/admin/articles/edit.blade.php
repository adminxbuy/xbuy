...
<div class="p-6 max-w-6xl mx-auto" x-data="{ 
    ...
    <div class="flex items-center space-x-3 mb-6">
        <a href="{{ route('admin.articles.index') }}" class="p-2 text-muted-foreground hover:text-foreground hover:bg-muted rounded-lg transition-all">
            <i data-lucide="arrow-left" class="w-5 h-5"></i>
        </a>
        <div>
            <h1 class="text-2xl font-bold text-foreground tracking-tight">Edit Article</h1>
            <p class="text-muted-foreground text-sm">Update content, fine-tune SEO parameters, and republish.</p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.articles.update', $article->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                <!-- Core Details Block -->
                <div class="bg-card border border-border rounded-xl p-6 space-y-4 shadow-sm">
                    <div>
                        ...
                        <input type="text" name="title" x-model="title" required placeholder="e.g. 5 Tips for Building Your PC"
                               class="w-full px-4 py-2.5 border border-border rounded-xl focus:outline-none focus:ring-1 focus:ring-ring focus:border-ring focus:border-transparent font-medium">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="slug" x-model="slug" required placeholder="custom-url-path"
                                   class="w-full px-4 py-2.5 border border-border rounded-xl focus:outline-none focus:ring-1 focus:ring-ring focus:border-ring focus:border-transparent text-sm">
                ...
                <div class="bg-card border border-border rounded-xl overflow-hidden shadow-sm flex flex-col min-h-[400px]">
                    <!-- Editor Tabs Toolbar -->
                    <div class="bg-muted border-b border-border px-4 py-2 flex items-center justify-between">
                        <div class="flex space-x-1">
                            <button type="button" @click="activeTab = 'write'" 
                                    ...
                                    :class="activeTab === 'write' ? 'bg-card shadow-sm border border-border text-foreground' : 'text-muted-foreground hover:text-foreground'">
                                Write (HTML/Standard)
                            </button>
                            <button type="button" @click="activeTab = 'preview'" 
                                    class="px-4 py-1.5 rounded-lg text-xs font-bold transition-all"
                                    :class="activeTab === 'preview' ? 'bg-card shadow-sm border border-border text-foreground' : 'text-muted-foreground hover:text-foreground'">
                                Live HTML Preview
                            </button>
                        </div>
            ...
            <div class="space-y-6">
                <!-- Cover Image Block -->
                <div class="bg-card border border-border rounded-xl p-6 space-y-4 shadow-sm" x-data="{ imageType: '{{ filter_var($article->cover_image, FILTER_VALIDATE_URL) ? 'url' : 'file' }}' }">
                    ...
                    @if($article->cover_image)
                        <div class="w-full aspect-[16/9] border border-border rounded-xl overflow-hidden mb-2 bg-muted">
                            <img src="{{ $article->cover_image }}" class="w-full h-full object-cover">
                        </div>
                    @endif

                    <div x-show="imageType === 'file'">
                        <input type="file" name="cover_image_file" class="w-full text-xs text-muted-foreground file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-muted file:text-foreground hover:file:bg-muted">
                    </div>
                    ...
                    <div x-show="imageType === 'url'">
                        <input type="text" name="cover_image_url" value="{{ filter_var($article->cover_image, FILTER_VALIDATE_URL) ? $article->cover_image : '' }}" placeholder="https://example.com/image.jpg"
                               class="w-full px-3 py-2 border border-border rounded-xl text-xs focus:outline-none focus:ring-1 focus:ring-ring focus:border-ring">
                    </div>
                </div>

                <!-- SEO Optimization Block -->
                <div class="bg-card border border-border rounded-xl p-6 space-y-4 shadow-sm">
                    <h3 class="text-sm font-bold text-foreground uppercase tracking-wider pb-2 border-b border-border">SEO Settings</h3>
                    <div>
                    ...
                    <div>
                        ...
                        <textarea name="seo_description" rows="3" placeholder="Brief search snippet description..."
                                  class="w-full px-3 py-2 border border-border rounded-xl text-xs focus:outline-none focus:ring-1 focus:ring-ring focus:border-ring resize-none">{{ $article->seo_description }}</textarea>
                    </div>
                </div>
                ...
                <div class="bg-card border border-border rounded-xl p-6 space-y-4 shadow-sm">
                    ...
                    <div class="flex items-center justify-between" x-data="{ isPublished: {{ $article->is_published ? 'true' : 'false' }} }">
                        <span class="text-xs font-semibold text-foreground">Published Status</span>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1.5">Scheduled Date & Time</label>
                        <input type="text" name="published_at" value="{{ $article->published_at ? $article->published_at->format('Y-m-d H:i') : '' }}"
                               class="flatpickr-datetime w-full px-3 py-2 border border-border rounded-xl text-xs focus:outline-none focus:ring-1 focus:ring-ring focus:border-ring" placeholder="Select date & time">
                    </div>

                    <button type="submit" class="w-full bg-primary text-primary-foreground hover:bg-primary/90 font-bold py-2.5 rounded-xl text-sm transition-all shadow-md active:scale-95">
                        Update Article
                    </button>
                </div>
            </div>
