...
@section('header_actions')
 <a href="{{ route('admin.trash.index', 'subscribers') }}" class="flex items-center space-x-1.5 px-4 py-2.5 bg-muted hover:bg-muted text-foreground rounded-lg border border-border/50 text-xs font-medium transition-all border border-border ">
...
@section('content')
<div class="space-y-6" x-data="{  tab: '{{ request('tab', 'subscribers') }}',
 selectedIds: [],  selectAll: false,  bulkAction: '',
 htmlContent: '',
...
">
 ...
 @if(session('info'))
 <div class="bg-muted border border-border text-foreground rounded-xl p-4 text-xs font-semibold">
 {{ session('info') }}
 </div>
 @endif
 @if(session('success'))
 <div class="bg-muted border border-emerald-250 text-foreground rounded-xl p-4 text-xs font-semibold">
 {{ session('success') }}
 </div>
 @endif
 ...
 <div class="bg-card border border-border rounded-xl p-4 flex flex-col md:flex-row gap-3 items-center justify-between">
 ...
 <input type="text" name="search" value="{{ request('search') }}" placeholder="Search email or name..."  class="w-full pl-9 pr-4 py-2 border border-border rounded-lg text-xs focus:outline-none focus:ring-1 focus:ring-ring focus:border-ring">
 ...
 @else
 <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-muted text-foreground border border-border/60">
 Self Subscribed
 </span>
 @endif
 ...
 <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-muted text-foreground">Active</span>
 @else
 <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-muted text-foreground">Unsubscribed</span>
 @endif
 </td>
 ...
 <td class="py-3.5 px-4">
 @if($camp->status === 'sent')
 <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-muted text-foreground border border-border/60">Sent</span>
 @elseif($camp->status === 'scheduled')
 <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-muted text-foreground border border-border/60 animate-pulse">Scheduled</span>
 @elseif($camp->status === 'sending')
 <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-muted text-foreground">Sending...</span>
 @elseif($camp->status === 'partial')
 <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-muted text-foreground border border-border/60">Partial</span>
 @else
 <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-muted text-foreground">Failed</span>
 @endif
 </td>
 <td class="py-3.5 px-4 text-xs text-muted-foreground font-medium">
 @if($camp->status === 'scheduled' && $camp->scheduled_at)
 <div class="text-muted-foreground font-bold">Queue:</div>
 ...
 <div class="w-full lg:w-1/4 shrink-0 bg-card border border-border rounded-xl p-4 h-fit space-y-1">
 ...
 <div class="flex-1 bg-card border border-border rounded-xl p-6 space-y-6">
 ...
 <div x-show="showAddModal"  class="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm"
 ...
 <!-- Feedback alerts -->
 <div x-show="importSuccessMsg" class="bg-muted border border-border text-foreground text-xs rounded-xl p-3" x-text="importSuccessMsg"></div>
 <div x-show="importErrorMsg" class="bg-muted border border-border text-foreground text-xs rounded-xl p-3" x-text="importErrorMsg"></div>

 <!-- Search user box -->
 <div class="relative">
 <input type="text" x-model="userSearchQuery" placeholder="Search system users by name or email..."  class="w-full pl-9 pr-4 py-2 border border-border rounded-lg text-xs focus:outline-none focus:ring-1 focus:ring-ring focus:border-ring">
 <i data-lucide="search" class="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground w-3.5 h-3.5"></i>
 </div>
