...
<div class="space-y-6" x-data="adminAccountsPage()">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4 bg-white border border-zinc-200 rounded-xl p-4 shadow-sm">
        <div class="flex items-center gap-3">
            <div class="p-2.5 bg-zinc-50 text-zinc-700 rounded-xl border border-zinc-100">
                <i data-lucide="users-round" class="w-5 h-5"></i>
            </div>
            <div>
                <h4 class="font-bold text-zinc-950 text-sm">Staff Management</h4>
                <p class="text-xs text-zinc-500">Create, update roles, and revoke staff access. Only Super Admins can manage this section.</p>
            </div>
        </div>
        <a href="{{ route('admin.accounts.create') }}"
           class="px-4 py-2.5 bg-zinc-900 text-white hover:bg-zinc-800 hover:bg-[#e6c22f] text-black rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all shadow-sm">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-3">
        ...
        <a href="{{ route('admin.accounts.index', ['admin_role' => $roleKey]) }}"
           class="bg-white border {{ request('admin_role') === $roleKey ? 'border-yellow-400 ring-2 ring-yellow-150' : 'border-zinc-200' }} rounded-xl p-3 shadow-sm hover:shadow-md transition-all flex flex-col items-center gap-2 text-center">
            ...
            <div>
                <p class="text-[10px] font-bold text-zinc-500">{{ $rc['label'] }}</p>
                <p class="text-lg font-black text-zinc-900">{{ $roleCounts[$roleKey] ?? 0 }}</p>
            </div>
        </a>
        @endforeach
    </div>
    ...
    <div class="bg-white border border-zinc-200 rounded-xl p-4 shadow-sm">
        <form action="{{ route('admin.accounts.index') }}" method="GET" class="flex flex-wrap items-end gap-4">
            <div class="w-full sm:w-64 space-y-1">
                <label class="text-[10px] font-bold text-zinc-500 uppercase tracking-wider block">Search</label>
                <div class="relative">
            ...
            <div class="w-full sm:w-48 space-y-1">
                <label class="text-[10px] font-bold text-zinc-500 uppercase tracking-wider block">Role</label>
                <div class="relative">
                    <select name="admin_role" class="w-full p-2.5 pl-3 pr-8 text-xs border border-zinc-200 rounded-xl bg-zinc-50 focus:bg-white focus:ring-1 focus:ring-zinc-950 focus:border-zinc-950 focus:outline-none transition-all cursor-pointer font-semibold text-zinc-700 appearance-none">
            ...
            <div class="flex items-center gap-2 pt-5">
                <button type="submit" class="px-4 py-2.5 bg-zinc-900 text-white rounded-xl text-xs font-semibold flex items-center gap-1.5 hover:bg-zinc-700 transition-all">
                    <i data-lucide="filter" class="w-3.5 h-3.5"></i> Filter
                </button>
                @if(request()->anyFilled(['search','admin_role']))
                    <a href="{{ route('admin.accounts.index') }}" class="px-4 py-2.5 border border-zinc-200 text-zinc-650 hover:bg-zinc-50 rounded-xl text-xs font-semibold transition-all">Clear</a>
                @endif
            </div>
        </form>
    </div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-zinc-100 bg-zinc-50/60">
                        <th class="text-left px-5 py-3 text-[10px] font-black text-zinc-400 uppercase tracking-widest">Staff Member</th>
                        <th class="text-left px-4 py-3 text-[10px] font-black text-zinc-400 uppercase tracking-widest">Role</th>
                        <th class="text-left px-4 py-3 text-[10px] font-black text-zinc-400 uppercase tracking-widest">Access</th>
                        <th class="text-left px-4 py-3 text-[10px] font-black text-zinc-400 uppercase tracking-widest">Joined</th>
                        <th class="text-right px-5 py-3 text-[10px] font-black text-zinc-400 uppercase tracking-widest">Actions</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-zinc-100">
                    @forelse($admins as $admin)
                        ...
                        <tr class="hover:bg-zinc-50/40 transition-all">
                            <td class="px-5 py-3.5">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-[10px] text-zinc-400">{{ $admin->email }}</p>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-4 py-3.5">
                                <p class="text-[10px] text-zinc-500 max-w-[200px] truncate" title="{{ $accessStr }}">
                                    {{ $accessStr }}
                                </p>
                            </td>
                            <td class="px-4 py-3.5">
                                <span class="text-[10px] text-zinc-400">{{ $admin->created_at->format('d M Y') }}</span>
                            </td>
                                ...
                                <div class="flex items-center justify-end gap-2">
                                    @if($admin->id !== Auth::id())
                                        ...
                                        <a href="{{ route('admin.accounts.edit', $admin->id) }}"
                                           class="px-3 py-1.5 bg-zinc-100 hover:bg-zinc-200 border border-zinc-200 text-zinc-700 rounded-lg text-[10px] font-bold flex items-center gap-1 transition-all">
                                    ...
                                    @else
                                        <span class="text-[10px] text-zinc-400 bg-zinc-50 border border-zinc-200 px-2.5 py-1 rounded-lg">Your account</span>
                                    @endif
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="py-16 text-center">
                                <div class="w-12 h-12 rounded-xl bg-zinc-50 border border-zinc-100 flex items-center justify-center mx-auto mb-3 text-zinc-650">
                                    <i data-lucide="users-round" class="w-5 h-5"></i>
                                </div>
                                <h4 class="text-sm font-bold text-zinc-800">No Staff Accounts Found</h4>
                                <p class="text-xs text-zinc-400 mt-1">No staff members match your current filters.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
