...
@section('header_actions')
    <a href="{{ route('admin.trash.index', 'escrows') }}" class="flex items-center space-x-1.5 px-4 py-2.5 rounded-lg border text-xs font-medium transition-all shadow-sm bg-rose-50 hover:bg-rose-100 text-rose-700 border-rose-200 dark:bg-rose-950/60 dark:hover:bg-rose-900 dark:text-rose-300 dark:border-rose-900">
...
<div class="space-y-6 w-full" x-data="escrowPage()">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <!-- Card 1: Total Held -->
        <div class="rounded-xl p-5 shadow-sm flex flex-col gap-2 border border-border bg-card">
            <div class="flex items-center justify-between">
                <div class="p-2 bg-muted text-foreground rounded-xl"><i data-lucide="lock" class="w-5 h-5"></i></div>
                <span class="text-[10px] font-bold px-2 py-0.5 rounded-full border bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950/60 dark:text-amber-300 dark:border-amber-900">{{ $summary['count_held'] }} held</span>
            </div>
        ...
        <!-- Card 2: Due Today -->
        <div class="rounded-xl p-5 shadow-sm flex flex-col gap-2 border border-border bg-card">
            <div class="p-2 rounded-xl w-fit bg-blue-50 text-blue-700 dark:bg-blue-950/60 dark:text-blue-300"><i data-lucide="clock" class="w-5 h-5"></i></div>
        ...
        <!-- Card 3: Overdue -->
        <div class="rounded-xl p-5 shadow-sm flex flex-col gap-2 border border-border bg-card">
            <div class="p-2 rounded-xl w-fit bg-amber-50 text-amber-700 dark:bg-amber-950/60 dark:text-amber-300"><i data-lucide="alert-circle" class="w-5 h-5"></i></div>
        ...
        <div class="rounded-xl p-5 shadow-sm flex flex-col gap-2 border border-border bg-card">
            <div class="p-2 rounded-xl w-fit bg-rose-50 text-rose-700 dark:bg-rose-950/60 dark:text-rose-300"><i data-lucide="help-circle" class="w-5 h-5"></i></div>
    ...
    <!-- Filters Panel -->
    <div class="rounded-xl p-5 shadow-sm border border-border bg-card">
    ...
    <div class="rounded-xl ring-0 overflow-hidden border border-border bg-card">
        <!-- Bulk Action Toolbar -->
        <div x-show="selectedIds.length > 0" x-transition.opacity style="display: none;" class="p-3 flex items-center justify-between border-b bg-amber-50 border-amber-200 dark:bg-amber-950/60 dark:border-amber-900">
            <div class="flex items-center space-x-3">
                <span class="font-bold text-xs uppercase tracking-wider px-3 py-1 rounded-lg bg-amber-100 text-amber-800 dark:bg-amber-900 dark:text-amber-200"><span x-text="selectedIds.length"></span> Selected</span>
            ...
            <button type="button" @click="selectedIds = []; selectAll = false" class="text-xs font-bold px-2 py-1 uppercase tracking-wider text-amber-700 hover:text-amber-900 dark:text-amber-300 dark:hover:text-amber-100">Clear</button>
        </div>
            ...
            <table class="w-full text-left border-collapse text-sm">
                ...
                <tbody class="divide-y divide-border">
                    @forelse($escrows as $escrow)
                        ...
                        @endphp
                        <tr class="transition-colors hover:bg-muted {{ $isOverdue ? 'bg-amber-50/40 hover:bg-amber-50/60 dark:bg-amber-950/30 dark:hover:bg-amber-950/50' : '' }}">
                                ...
                                <div class="flex items-center space-x-2">
                                    ...
                                    @if($escrow->order_id)
                                        <a href="{{ route('admin.orders.show', $escrow->order_id) }}" target="_blank" @click.stop
                                           class="inline-flex items-center justify-center p-1 rounded-lg shadow-sm transition-all shrink-0 border border-border bg-muted text-muted-foreground hover:bg-muted"
                            ...
                            <!-- Release Scheduled -->
                            <td class="p-4 text-xs font-semibold {{ $isOverdue ? 'text-amber-700 dark:text-amber-400' : 'text-muted-foreground' }}">
                            ...
                            <td class="p-4 text-center">
                                <span class="px-2 py-0.5 text-[10px] rounded-full font-bold uppercase tracking-wider border
                                    @if($escrow->status === 'held') bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950/60 dark:text-amber-300 dark:border-amber-900
                                    @elseif($escrow->status === 'released') bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-950/60 dark:text-emerald-300 dark:border-emerald-900
                                    @elseif($escrow->status === 'partially_released') bg-purple-50 text-purple-700 border-purple-200 dark:bg-purple-950/60 dark:text-purple-300 dark:border-purple-900
                                    @elseif($escrow->status === 'refunded') bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-950/60 dark:text-blue-300 dark:border-blue-900
                                    @elseif($escrow->status === 'disputed') bg-rose-50 text-rose-700 border-rose-200 dark:bg-rose-950/60 dark:text-rose-300 dark:border-rose-900
                            ...
                            <td class="p-4 pr-6 text-right space-x-1 whitespace-nowrap">
                                @if($escrow->status === 'held' || $escrow->status === 'disputed')
                                    <form action="{{ route('admin.escrow.release', $escrow->id) }}" method="POST" class="inline" onsubmit="return confirm('Release this escrow fully to the seller?');">
                                        @csrf
                                        <button type="submit" class="text-[10px] font-medium px-2 py-1.5 rounded-lg transition-all border bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100 dark:bg-emerald-950/60 dark:text-emerald-300 dark:border-emerald-900 dark:hover:bg-emerald-900" title="Release to Seller">
                                            Release
                                        </button>
                                    </form>
                                    ...
                                    <form action="{{ route('admin.escrow.refund', $escrow->id) }}" method="POST" class="inline" onsubmit="return confirm('Refund this escrow fully to the buyer?');">
                                        @csrf
                                        <button type="submit" class="text-[10px] font-medium px-2 py-1.5 rounded-lg transition-all border bg-rose-50 text-rose-700 border-rose-200 hover:bg-rose-100 dark:bg-rose-950/60 dark:text-rose-300 dark:border-rose-900 dark:hover:bg-rose-900" title="Refund to Buyer">
                                            Refund
                                        </button>
                                    </form>

                                    <button @click="openPartialModal({{ json_encode($escrow) }})" class="text-[10px] font-bold px-2 py-1.5 rounded-lg transition-all border bg-purple-50 text-purple-700 border-purple-200 hover:bg-purple-100 dark:bg-purple-950/60 dark:text-purple-300 dark:border-purple-900 dark:hover:bg-purple-900" title="Partial Escrow Release">
                                        Partial
                                    </button>
                                @else
                        ...
                        <tr x-show="isExpanded({{ $escrow->id }})" x-cloak class="bg-muted">
                            <td colspan="9" class="p-5 pl-8 pr-6 border-b border-border">
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 text-xs p-5 shadow-sm animate-fade-in rounded-xl text-muted-foreground bg-card border border-border">
                                    ...
                                    <div class="space-y-3">
                                        ...
                                        <div class="space-y-2.5">
                                            <div class="p-2.5 rounded-xl border bg-rose-50 border-rose-200 dark:bg-rose-950/60 dark:border-rose-900">
                                                <p class="font-bold mb-0.5 text-rose-800 dark:text-rose-300">Buyer: {{ $escrow->order->buyer->name ?? 'N/A' }}</p>
                                            ...
                                            <div class="p-2.5 rounded-xl border bg-blue-50 border-blue-200 dark:bg-blue-950/60 dark:border-blue-900">
                                                <p class="font-bold mb-0.5 text-blue-800 dark:text-blue-300">Seller Shop: {{ $escrow->order->seller->shop_name ?? 'N/A' }}</p>
                                    ...
                                    <div class="space-y-3">
                                        ...
                                        <div class="space-y-2">
                                            ...
                                            <p><span class="font-semibold text-muted-foreground">Payout Status:</span>
                                                <span class="font-bold @if($escrow->payout_status === 'success') text-emerald-600 dark:text-emerald-400 @elseif($escrow->payout_status === 'failed') text-rose-600 dark:text-rose-400 @else text-muted-foreground @endif">{{ ucfirst($escrow->payout_status ?? 'pending') }}</span>
                                            </p>
                                            @if($escrow->payout_status === 'failed')
                                                <p class="leading-tight mt-0.5 text-rose-600 dark:text-rose-400"><span class="font-bold">Error:</span> <span>{{ $escrow->payout_error_message }}</span></p>
                                                <form action="{{ route('admin.escrow.retry-payout', $escrow->id) }}" method="POST" class="mt-1">
                                                    @csrf
                                                    <button type="submit" class="text-[9px] font-bold px-2 py-1 rounded-md transition-all border bg-amber-50 text-amber-800 border-amber-200 hover:bg-amber-100 dark:bg-amber-950/60 dark:text-amber-300 dark:border-amber-900 dark:hover:bg-amber-900">
        ...
        <div class="relative w-full max-w-md mx-auto rounded-[24px] shadow-2xl z-10 overflow-hidden bg-card border border-border"
            ...
            <form :action="'/admin/escrow/' + selectedEscrow.id + '/partial-release'" method="POST" class="p-6 space-y-4 text-sm">
                ...
                <div class="pt-4 border-t flex justify-end space-x-2 border-border">
                    ...
                    <button type="submit" class="px-5 py-2 font-semibold rounded-lg text-xs shadow-sm transition-all bg-primary text-primary-foreground">
                        Release Partially
                    </button>
                </div>
            </form>
