...
@section('header_actions')
    <a href="{{ route('admin.staff.directory') }}" class="flex items-center space-x-1.5 px-3 py-1.5 bg-muted hover:bg-muted text-foreground hover:text-foreground rounded-lg text-xs font-semibold transition-all border border-border shadow-sm">
...
<div x-data="{ 
    ...
    <div class="lg:col-span-1 space-y-6">
        <div class="bg-card border border-border rounded-xl p-6 shadow-sm text-center relative overflow-hidden">
            ...
            <img src="https://ui-avatars.com/api/?name={{ urlencode($member->name) }}&background=e4e4e7&color=71717a&size=96" 
                 class="w-24 h-24 rounded-xl mx-auto border-2 border-border shadow-sm mb-4" alt="avatar">
    ...
    <div class="lg:col-span-3">
        ...
        <div class="bg-muted p-1.5 rounded-xl flex flex-wrap gap-1 mb-6 border border-border">
            <button @click="activeTab = 'profile'" 
                    :class="activeTab === 'profile' ? 'bg-card text-foreground font-bold shadow-sm' : 'text-muted-foreground hover:text-foreground font-semibold'"
            ...
            <button @click="activeTab = 'earnings'" 
                    :class="activeTab === 'earnings' ? 'bg-card text-foreground font-bold shadow-sm' : 'text-muted-foreground hover:text-foreground font-semibold'"
                    class="flex-1 px-4 py-2.5 rounded-xl text-xs transition-all flex items-center justify-center gap-2">
                <i data-lucide="wallet" class="w-4 h-4"></i>
                <span>Earnings History</span>
            </button>
            <button @click="activeTab = 'notices'" 
                    :class="activeTab === 'notices' ? 'bg-card text-foreground font-bold shadow-sm' : 'text-muted-foreground hover:text-foreground font-semibold'"
                    class="flex-1 px-4 py-2.5 rounded-xl text-xs transition-all flex items-center justify-center gap-2">
                <i data-lucide="clipboard-list" class="w-4 h-4"></i>
                <span>Notices Sent</span>
            </button>
            <button @click="activeTab = 'logs'" 
                    :class="activeTab === 'logs' ? 'bg-card text-foreground font-bold shadow-sm' : 'text-muted-foreground hover:text-foreground font-semibold'"
                    class="flex-1 px-4 py-2.5 rounded-xl text-xs transition-all flex items-center justify-center gap-2">
        ...
        <!-- Tab 1 — Profile Info -->
        <div x-show="activeTab === 'profile'" class="bg-card border border-border rounded-xl p-6 shadow-sm space-y-6">
            <div>
        ...
        <div x-show="activeTab === 'earnings'" class="bg-card border border-border rounded-xl p-6 shadow-sm space-y-6">
                        ...
                        <tr class="hover:bg-muted/50 transition-all font-semibold">
                            ...
                            <td class="px-6 py-3.5 text-right">
                                <button @click="expandedMonths['{{ $sum['month'] }}'] = !expandedMonths['{{ $sum['month'] }}']" 
                                        class="text-xs font-bold bg-muted hover:bg-primary text-primary-foreground hover:bg-primary/90 text-foreground px-3 py-1 rounded-lg transition-all border border-border">
        ...
        <!-- Tab 3 — Notices Sent -->
        <div x-show="activeTab === 'notices'" class="bg-card border border-border rounded-xl p-6 shadow-sm space-y-6">
            <div>
        ...
        <!-- Tab 4 — Activity Log -->
        <div x-show="activeTab === 'logs'" class="bg-card border border-border rounded-xl p-6 shadow-sm space-y-6">
            <div>
