...
<div class="max-w-7xl mx-auto px-4 sm:px-6 py-12 space-y-12">
    ...
    <!-- Featured Post -->
    @if($featuredArticle)
        <a href="{{ route('blog.show', $featuredArticle->slug) }}" class="block bg-white border border-zinc-200 rounded-3xl overflow-hidden shadow-sm hover:shadow-md transition-all group duration-300">
            <div class="grid grid-cols-1 lg:grid-cols-2">
                ...
                <div class="aspect-[16/10] lg:aspect-auto w-full bg-zinc-100 overflow-hidden relative">
                    ...
                    @endif
                    <span class="absolute top-4 left-4 bg-[#fdd835] text-black font-semibold text-[10px] px-3 py-1.5 rounded-full uppercase tracking-wider shadow-sm">Featured</span>
                </div>
                <!-- Content -->
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 pt-4 border-t border-zinc-200">
        ...
        <div class="flex flex-wrap items-center gap-2">
            ...
            @foreach($categories as $cat)
                <a href="{{ route('blog.index', ['category' => $cat]) }}" 
                   class="px-4 py-2 text-xs font-bold rounded-full transition-all border {{ request('category') === $cat ? 'bg-black text-white border-black' : 'bg-white text-zinc-700 border-zinc-200 hover:border-zinc-300' }}">{{ $cat }}</a>
            @endforeach
        </div>
        ...
        <form action="{{ route('blog.index') }}" method="GET" class="relative max-w-sm w-full">
            ...
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search articles..." 
                   class="w-full pl-9 pr-4 py-2 border border-zinc-200 rounded-full text-xs focus:outline-none focus:ring-2 focus:ring-[#fdd835]">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        @forelse($articles as $article)
            <article class="bg-white border border-zinc-200 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all group flex flex-col justify-between">
                <div>
                    <!-- Image -->
